...
class Decoder(object):
  ...
  @abc.abstractmethod
  def step(self, time, inputs, state,full, name=None):
...
def dynamic_decode(decoder,win,enc_win,hidden,outputs_enc,max_vertices,
  ...
  with variable_scope.variable_scope(scope, "decoder") as varscope:
    ...
    initial_finished, initial_inputs, initial_state,ini_state,ini_att = decoder.initialize()

    paddings=tf.constant([[0,0],[0,hidden]])
    initial_inputs=tf.pad(initial_inputs,paddings)
    ...
    def condition(unused_time, unused_outputs_ta, unused_state, unused_inputs,
                  finished, unused_sequence_lengths,full,all_att):
      return math_ops.logical_not(math_ops.reduce_all(finished))

    def body(time, outputs_ta, state, inputs, finished, sequence_lengths,full,all_att):
      ...
      (next_outputs, decoder_state, next_inputs,
       decoder_finished,decoder_state_full,b_att) = decoder.step(time, inputs, state,full,outputs_enc,hidden,all_att)
      #full_States.append(decoder_state)
      ...
      return (time + 1, outputs_ta, next_state, next_inputs, next_finished,
              next_sequence_lengths,decoder_state_full,b_att)
    ...
    all_states_dec = tf.slice(all_states, [1, 0], [-1, -1])
    batch_all_att=tf.slice(batch_att,[1,0],[-1,-1])

    all_states_dec=tf.reshape(all_states_dec,shape=(-1,win,hidden))
    batch_all_att=tf.reshape(batch_all_att,shape=(-1,win,enc_win))

  return final_outputs, final_state, final_sequence_lengths,all_states_dec,batch_all_att
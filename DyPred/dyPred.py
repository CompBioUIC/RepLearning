...
class Model(object):
    ...
    def make_model(self):  # this is a general model
        ...
        self.placeholders['ini_att']=tf.placeholder(tf.float32,[None,self.params['win']],name='ini_att')
        ...
        grRep=self.compressGr(self.ops['final_node_representations'])
        grRep=tf.reshape(grRep,[-1,self.params["win"],self.params['hidden_size']])
        ...
        decoder = BasicDecoder(dec_cell, helper, encoder_state,self.placeholders['ini_state'],self.placeholders['ini_att'],output_layer)
        outputs_dec, state_Dec, _ ,full_States,batch_all_att= dynamic_decode(decoder,max_target_len,self.params["win"],self.params["hidden_size"],outputs_enc,self.max_num_vertices, impute_finished=True,maximum_iterations=max_target_len)
        self.placeholders['dec_output']=outputs_dec
        self.placeholders['state_Dec']=full_States
        self.placeholders['att']=batch_all_att


        masks = tf.sequence_mask(self.placeholders['target_sequence_length'], max_target_len, dtype=tf.float32,name='masks')#b,tl(w)
        masks=tf.expand_dims(masks,2)#b,tl,1
    ...
    def make_train_step(self):# configure the optimizer
    ...
    def writeGrRepSVMformat(self,grReps,data,name,type,training):
        ...
        for i in range(len(grReps)):
            if(type=="dec"):
                k=i+self.params["win"]
            else:
                k=i
            if(k<whole_nu_grs and i<len(grReps)):
                grRep = grReps[i]
                fw.write(str(data[k]["targets"]))
    ...
    def writeAttBatch(self,fw,batch_att,start,step):
        for seq in batch_att:
            for att in seq:
                fw.write(str(start) + " ")
    ...
    def findRep(self,data,name,training):
        batch_iterator = ThreadedIterator(self.make_minibatch_iterator(data, False), max_queue_size=1)
        grReps_enc=[]
        grReps_dec=[]
        grReps_static=[]
        step_enc = 0
        ...
        for batch_ind, batch_data in enumerate(batch_iterator):
            enc_output,static_grRep,_,state_dec,batch_att = self.sess.run((self.placeholders['enc_output'],self.placeholders['static_gr_rep'],self.placeholders['dec_output'],self.placeholders['state_Dec'],self.placeholders['att']), feed_dict=batch_data)
    ...
    def run_epoch(self, epoch_name: str, data):
        ...
        processed_graphs = 0
        batch_iterator = ThreadedIterator(self.make_minibatch_iterator(data, True), max_queue_size=1)

...
from dyModel import Model
from utils import glorot_init

def graph_to_adj(graph, max_n_vertices, num_edge_types, tie_fwd_bkwd=True):
...
class GGNNModel(Model):
    ...
    def compressGr(self,last_h):
        last_h=tf.reduce_mean(last_h,axis=1)
        return last_h
    ...
    def process_raw_Graphs(self, raw_data: Sequence[Any], is_training_data: bool, bucket_sizes=None) -> Any:
        ...
        for i in range(len(raw_data)):
            d=raw_data[i]
            adjMat,target=graph_to_adj(d['graph'], self.max_num_vertices, self.num_edge_types, self.params['tie_fwd_bkwd'])
            bucketed[i]={
                'adj_mat': adjMat,
                'labels': target,
                'targets':d["targets"]
    ...
    def make_batch(self, elements,end_data):
        batch_data = {'adj_mat': [], 'labels': [],'dec_input':[]}
        last_seq_index_in_batch = len(elements) - 1
        for seq_ind,seq in enumerate(elements):
            ...
            for ind_dy_graph,d in enumerate(seq):
                ...
                target_task_values=d['labels']
                batch_data['labels'].append(target_task_values)
                if(end_data==True and seq_ind==last_seq_index_in_batch):
                    batch_data['dec_input'].append(target_task_values)
                else:
                    if(ind_dy_graph<len(seq)-1):
                        batch_data['dec_input'].append(target_task_values)
        return batch_data
        ...
        while(jj<index):
            ...
            for i in range(len(amat[0])):
                for j in range(len(amat[0][i])):
                    target.append(0)
            target = np.asarray(target, dtype=np.float32)
            batch_data['adj_mat'].append(amat)
    ...
    def make_minibatch_iterator(self, data, is_training: bool):
        ...
        step=0
        end_idx=0
        while(step<len(data) and end_idx<len(data)):
            elements_batch=[]
            timeSteps_batch=[]
            num_graphs=0
            end_data=False
            for b_nu in range(self.params['batch_size']):
                ...
                for ind in range(start_idx, end_idx):
                    if (ind < len(data)):
                        ...
                        index=end_idx-ind
                        len_last_seq_in_batch=ind-start_idx
                        end_data=True
                        break
                step=step+self.params["overlap"]
                elements_batch.append(elements)
                timeSteps_batch.append(len(elements))
                if(end_data==True):
                    break
            if (end_data == False):
                index=self.params["win"]


            for seq in elements_batch:
                num_graphs=num_graphs+len(seq)

            batch_data = self.make_batch(elements_batch,end_data)

            if(end_data == True):
                batch_data=self.make_pad(batch_data,index,len_last_seq_in_batch,max(timeSteps_batch))
                num_graphs_init=num_graphs+index
            ...
            batch_feed_dict = {
                ...
                self.placeholders['target_sequence_length']:timeSteps_batch,
...
def main():
    ...
    try:
        ...
        optparser.add_option("-w", "--win", default="10",type='int',help="Window size")
        ...
        optparser.add_option("-e", "--epoch", default="10",type='int',help="Number of epochs")
        ...
        optparser.add_option("-o", "--overlap", default="5",type='int', help="the overlap that we need to slid the window; it should be less than window size")
